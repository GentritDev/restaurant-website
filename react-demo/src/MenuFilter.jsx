...
// 1. Extracted MenuItem for cleaner code
  ...
  <div style={{ background: '#1f2024', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '8px', padding: '1rem' }}>
    ...
    <button 
      ...
      style={{ 
        width: '100%', 
        padding: '8px', 
        background: isFavorite ? '#12b981' : '#202127', 
        color: '#fff', 
        border: `1px solid ${isFavorite ? '#12b981' : 'rgba(255,255,255,0.14)'}`, 
        borderRadius: '4px', 
        cursor: 'pointer', 
        fontWeight: '600',
        transition: 'all 0.2s'
...
const MenuFilter = () => {
  // 2. Optimization: Initialize state FROM LocalStorage so data persists on refresh
  const [filter, setFilter] = useState(() => localStorage.getItem('gourmet_menu_filter') || 'gjitha');
  ...
  // Removed syntax error "menuItems. filter" -> "menuItems.filter"
  ...
  return (
    <div style={{ maxWidth: '1120px', margin: '0 auto', padding: '2rem 1rem', fontFamily: 'system-ui, sans-serif' }}>
      <h2 style={{ fontFamily: 'Playfair Display, serif', fontSize: '2rem', marginBottom: '1rem', textAlign: 'center', color: '#202127' }}>
        🍽️ Filtri i Menysë
      </h2>
      ...
      <form onSubmit={handleSubmit} style={{ marginBottom: '2rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
        <div>
          ...
          <input 
            type="text" 
            placeholder="Kërko në meny..." 
            value={searchTerm} 
            onChange={(e) => setSearchTerm(e.target.value)} 
            style={{ width: '100%', padding: '10px', background: '#202127', color: '#fff', border: '1px solid rgba(255,255,255,0.14)', borderRadius: '6px' }} 
          />
        </div>
        <div>
          ...
          <select 
            ...
            style={{ width: '100%', padding: '10px', background: '#202127', color: '#fff', border: '1px solid rgba(255,255,255,0.14)', borderRadius: '6px', cursor: 'pointer' }}
          >
        ...
        <button type="submit" style={{ padding: '10px 20px', background: '#8f1d2c', color: '#fff', border: 'none', borderRadius: '6px', fontWeight: '600', cursor: 'pointer', alignSelf: 'flex-end', height: '42px' }}>
          💾 Ruaj Preferencat
        </button>
      </form>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '1rem' }}>
        {isLoading ? (
          <p style={{textAlign: 'center', width: '100%'}}>⏳ Po ngarkohet...</p>
          ...
          <p style={{textAlign: 'center', width: '100%'}}>❌ Nuk u gjet asnjë artikull</p>
        )}
      </div>
    </div>
  );
